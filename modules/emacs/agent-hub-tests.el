  ...
  (agent-hub-test--with-clean-state
    (agent-hub-test--in-fixture (home repo)
      (let* ((file1 (agent-hub-test--write-session home repo "dup" "First"))
             ...
             (signature (agent-hub--transcript-signature (seconds-to-time 1000) 10))
             (cache (agent-hub--title-cache-ensure))
             (plan (agent-hub--delete-plan (list session)))
             (result (progn
                       (puthash file1 (list :signature signature :metadata session) cache)
                       (puthash file2 (list :signature signature :metadata session) cache)
                       (agent-hub-test--delete-sessions (list session)))))
        (should (= (length (plist-get plan :files)) 2))
        (should-not (file-exists-p file1))
        (should-not (file-exists-p file2))
        (should-not (gethash file1 cache))
        (should-not (gethash file2 cache))
...
(ert-deftest agent-hub-test-transcript-cache-serves-stale-on-signature-change ()
  (agent-hub-test--with-clean-state
    (let* ((file "/repo/.agent-shell/transcripts/session.md")
           (old-time (seconds-to-time 1000))
           (new-time (seconds-to-time 1001))
           (old-signature (agent-hub--transcript-signature old-time 10))
           (metadata (list :file file :cwd "/repo" :time old-time
                           :title "Known title"))
           (entry (list :file file :cwd "/repo" :mtime new-time :size 11))
           (spawns 0)
           (agent-hub--async-runner
            (lambda (&rest _args) (setq spawns (1+ spawns)))))
      (puthash file (list :signature old-signature :metadata metadata)
               (agent-hub--title-cache-ensure))
      (let ((returned (agent-hub--transcript-metadata-cached entry)))
        (should (= spawns 1))
        (should (equal (plist-get returned :title) "Known title")))
      (let ((returned (agent-hub--transcript-metadata-cached entry)))
        (should (= spawns 1))
        (should (equal (plist-get returned :title) "Known title")))
      (should (equal (plist-get (gethash file (agent-hub--title-cache-ensure))
                                :metadata)
                     metadata)))))

(ert-deftest agent-hub-test-transcript-cache-converges-after-revalidation ()
  (agent-hub-test--with-clean-state
    (let* ((file "/repo/.agent-shell/transcripts/session.md")
           (old-time (seconds-to-time 1000))
           (new-time (seconds-to-time 1001))
           (old-signature (agent-hub--transcript-signature old-time 10))
           (new-signature (agent-hub--transcript-signature new-time 11))
           (metadata (list :file file :cwd "/repo" :time old-time
                           :title "Old title"))
           (entry (list :file file :cwd "/repo" :mtime new-time :size 11))
           callback)
      ...
      (cl-letf (((symbol-function 'agent-hub--schedule-rerender) #'ignore))
        (let ((agent-hub--async-runner
               (lambda (_key _dir _program _args cb) (setq callback cb))))
          (should (equal (plist-get (agent-hub--transcript-metadata-cached entry)
                                    :title)
                         "Old title"))
          (should callback)
          (funcall callback t agent-hub-test--transcript)
          (let ((cached (gethash file (agent-hub--title-cache-ensure))))
            (should (equal (plist-get cached :signature) new-signature))
            (should (string-prefix-p "Fix login"
                                     (plist-get (plist-get cached :metadata)
                                                :title)))))))))

(ert-deftest agent-hub-test-transcript-cache-failure-preserves-stale-title ()
  (agent-hub-test--with-clean-state
    (let* ((file "/repo/.agent-shell/transcripts/session.md")
           (old-time (seconds-to-time 1000))
           (new-time (seconds-to-time 1001))
      ...
      (cl-letf (((symbol-function 'agent-hub--schedule-rerender) #'ignore))
        (let ((agent-hub--async-runner
               (lambda (_key _dir _program _args cb)
          ...
          (let ((cached (gethash file (agent-hub--title-cache-ensure))))
            (should (equal (plist-get cached :signature) new-signature))
            (should (equal (plist-get (plist-get cached :metadata) :title)
                           "Known title")))
          (agent-hub--transcript-metadata-cached entry)
          (should (= spawns 1)))))))

(ert-deftest agent-hub-test-transcript-cache-negative-entry ()
  (agent-hub-test--with-clean-state
    (let* ((file "/repo/.agent-shell/transcripts/session.md")
           (old-time (seconds-to-time 1000))
           (new-time (seconds-to-time 1001))
           (old-signature (agent-hub--transcript-signature old-time 10))
           (entry (list :file file :cwd "/repo" :mtime old-time :size 10))
           (changed (list :file file :cwd "/repo" :mtime new-time :size 11))
           (spawns 0)
           (agent-hub--async-runner
            (lambda (&rest _args) (setq spawns (1+ spawns)))))
      (puthash file (list :signature old-signature :metadata nil)
               (agent-hub--title-cache-ensure))
      (let ((returned (agent-hub--transcript-metadata-cached entry)))
        (should (= spawns 0))
        (should (equal (plist-get returned :file) file))
        (should-not (plist-get returned :title)))
      (let ((returned (agent-hub--transcript-metadata-cached changed)))
        (should (= spawns 1))
        (should (equal (plist-get returned :file) file))
        (should-not (plist-get returned :title))))))

(ert-deftest agent-hub-test-session-files-does-not-prune-incomplete-listing ()
  (agent-hub-test--with-clean-state
    (let* ((root "/repo")
           (file "/repo/.agent-shell/transcripts/session.md")
           (signature (agent-hub--transcript-signature (seconds-to-time 1000) 10))
           (metadata (list :file file :cwd root :title "Known title")))
      (puthash file (list :signature signature :metadata metadata)
               (agent-hub--title-cache-ensure))
      (setq agent-hub--title-cache-dirty nil)
      (cl-letf (((symbol-function 'agent-hub--session-cwds) (lambda (_root) (list root)))
                ((symbol-function 'agent-hub--session-files-for-cwd) (lambda (_cwd) nil)))
        (should-not (agent-hub--session-files root)))
      (should (gethash file (agent-hub--title-cache-ensure)))
      (should-not agent-hub--title-cache-dirty))))

(ert-deftest agent-hub-test-force-refresh-preserves-title-cache ()
  (agent-hub-test--with-clean-state
    (let* ((file "/repo/.agent-shell/transcripts/session.md")
           (signature (agent-hub--transcript-signature (seconds-to-time 1000) 10))
           (metadata (list :file file :cwd "/repo" :title "Known title"))
           renders)
      (puthash file (list :signature signature :metadata metadata)
               (agent-hub--title-cache-ensure))
      (setq agent-hub--title-cache-dirty nil)
      (agent-hub--cache-put "git:/repo" '(:branch "main"))
      (cl-letf (((symbol-function 'agent-hub--render)
                 (lambda () (setq renders (1+ (or renders 0))))))
        (agent-hub-refresh t))
      (should (= renders 1))
      (should (equal (plist-get (plist-get (gethash file (agent-hub--title-cache-ensure))
                                           :metadata)
                                  :title)
                     "Known title"))
      (should (agent-hub--needs-fetch-p "git:/repo"))
      (should-not agent-hub--title-cache-dirty))))

(ert-deftest agent-hub-test-title-cache-ignores-legacy-entries ()
  (agent-hub-test--with-clean-state
    (let* ((old-file "/repo/.agent-shell/transcripts/old.md")
           (new-file "/repo/.agent-shell/transcripts/new.md")
           (signature (agent-hub--transcript-signature (seconds-to-time 1000) 10))
           (metadata (list :file new-file :cwd "/repo" :title "New title")))
      (with-temp-file agent-hub-title-cache-file
        (let ((print-length nil) (print-level nil))
          (prin1 (list (cons old-file (cons 1000.0 "Old title"))
                       (cons new-file (list :signature signature
                                            :metadata metadata)))
                 (current-buffer))))
      (let ((cache (agent-hub--title-cache-ensure)))
        (should-not (gethash old-file cache))
        (should (equal (plist-get (plist-get (gethash new-file cache) :metadata)
                                  :title)
                       "New title"))))))
